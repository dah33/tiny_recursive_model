...
def trunc_normal_init_(
    ...
    Copied from original TRM implementation (models/common.py).

    This is NOT simply clamping - it properly samples from a truncated normal distribution
    by using the inverse CDF method with error functions.
    """
    with torch.no_grad():
        if std == 0:
            tensor.zero_()
